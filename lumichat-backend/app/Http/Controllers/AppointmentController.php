...
class AppointmentController extends Controller
{
    ...
    /* ----------------------------------------------------------
     |  SLOTS (AJAX)
     |  GET /appointment/slots/{counselorId}?date=YYYY-MM-DD
     |----------------------------------------------------------*/
    public function slots($counselorId, Request $request)
    {
        ...
        if ($ranges->isEmpty()) {
            return response()->json([
                ...
                'message' => 'No counselor availability on that day. Please choose another date.',
            ]);
        }

        // Already booked times for this counselor on this date
    ...
    public function store(Request $request)
    {
        ...
        $scheduledAt = Carbon::parse($request->date.' '.$timeStr);
    ...
    /* ----------------------------------------------------------
     |  HISTORY LIST
     |  GET /appointment/history?status=...&period=...&q=...
     |  status: all|pending|confirmed|canceled|completed
     |  period: all|today|upcoming|this_week|this_month|past
     |----------------------------------------------------------*/
    ...
    public function show($id)
    {
        ...
        $appointment = DB::table('tbl_appointments as a')
            ...
            ->where('a.id', $id)
            ->where('a.student_id', $userId) // safety: only owner can view
            ->first();
    ...
    /**
     * Recheck whether the slot is available at the moment of booking.
     */
    private function isSlotAvailable(int $counselorId, Carbon $scheduledAt): bool
    {
        ...
        // Must fit in availability range
        $ranges = DB::table('tbl_counselor_availabilities')
            ->where('counselor_id', $counselorId)
            ->where('weekday', $dow)
            ->get(['start_time','end_time']);
