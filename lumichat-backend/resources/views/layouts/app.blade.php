...
<button id="sidebar-open" class="sidebar-open-btn" aria-label="Open sidebar">
  ...
  <div class="layout-wrapper">
    ...
    <aside id="sidebar"
      ...
      @php
        ...
        // Use your actual filenames from /public/images/icons (per your screenshot)
        $mainLinks = [
          'Home'         => ['chat.index',    'home.png'],
          'Profile'      => ['profile.edit',  'user.png'],
          'Chat History' => ['chat-history.png' /* route placeholder below */, 'chat-history.png'],
          'Settings'     => [null,            'settings.png'],   // no route yet -> disabled look
        ];

        // Fix the route name for Chat History if you have it registered:
        if (Route::has('chat.history')) {
            $mainLinks['Chat History'][0] = 'chat.history';
        }
      @endphp
      ...
      <nav class="flex-1 px-3 pt-5 space-y-4 overflow-y-auto">
        ...
        <div>
          ...
          <a href="{{ route('chat.new') }}"
            ...
            {{-- Use phone.png (your set) for New Chat --}}
  ...
  <script>
  // simple fallback toggle for the profile edit card
  document.addEventListener('DOMContentLoaded', function () {
