...
<div class="bg-white p-10 rounded-2xl shadow-xl w-full max-w-md animate__animated animate__fadeIn">
    ...
    {{-- Removed the inline $errors panel on purpose (SweetAlert will handle errors) --}}
...
<script>
  (function(){
    ...
    // SweetAlert for validation/auth errors (no inline panel)
    @if ($errors->any())
      (function(){
        const errors = @json($errors->all());
        const html = '<ul style="text-align:left;margin:0;padding-left:1rem">' + errors.map(e => `<li>• ${e}</li>`).join('') + '</ul>';
        Swal.fire({ icon:'error', title:'Incorrect Password', html, confirmButtonText:'OK' });
      })();
    @endif
